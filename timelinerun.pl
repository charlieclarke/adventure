...
sub run_timeline {
	    ...
	    my $sth = $db->prepare("select TimeLine.id, TimeLine.ThreadID, TimeLine.ActivityTime, TimeLine.Completed, TimeLine.CompletedTime, TimeLine.Description, TimeLine.Notes, Thread.ActionType, Thread.mp3Name, Thread.DestNumber, Thread.FrequencyMinutes,Thread.StartTimeHour, Thread.StopTimeHour,Thread.ChildThreadID, Number.NumberID, Number.Number, TNumber.TNumber, TNumber.TNumberID,TNumber.PrefixWL from TimeLine, Thread,Number, TNumber where TimeLine.Completed = 0 and Thread.TNumberID = TNumber.TNumberID and (TimeLine.ActivityTime < ? or TimeLine.ActivityTime is null) and TimeLine.ThreadID = Thread.id and TimeLine.AdditionalNumberID = Number.NumberID order by TimeLine.ActivityTime LIMIT 1");
		...
		if (scalar(@$all) > 0) {
			...
			#	print "got task $id with threadID $threadID: $description - actionType $actionType, mp3 $mp3Name addNumberID $additionalNumberID\n";
				...
				if ($actionType eq 1) {
					...
					#if the filter works, then we kickoff the children.
                                        filter_stash_equal_action ($additionalNumberID,$mp3Name,$threadID,$childThreadID);
			        } elsif ($actionType eq 25) { #filter if stash <. this is the odd one where we need <STASHNAME>,<value>
					#if the filter works, then we kickoff the children.
...
sub filter_stash_equal_action {
	my ($additionalNumberID,$mp3name,$threadID, $childThreadID) = @_;


	print "threadID $threadID equial to filter on $mp3name\n";
	if ($mp3name =~ /(\w+),(\d+)/) {
		$key = $1;
		$value = $2;	
	...
	#	$sth->execute();
		...
		print "FILTER: comparing is $stashvalue = for $mp3name\n"; 
...
sub filter_stash_greater_action {
	...
	print "threadID $threadID greater than filter on $mp3name\n";
	if ($mp3name =~ /(\w+),(\d+)/) {
		$key = $1;
		$value = $2;	
...
sub filter_stash_less_action { 
	...
	print "threadID $threadID less than filter on $mp3name\n";
	if ($mp3name =~ /(\w+),(\d+)/) {
		$key = $1;
		$value = $2;	
	...
	#	$sth->execute();
		...
		print "FILTER: comparing $stashvalue < $mp3name\n"; 
		...
		if ($stashvalue < $value) {
			...
			print "filter not pssed for -$stashvalue- < -$value-\n";
		}
	} else {
		print "FILTER - bad key\n";
	}
