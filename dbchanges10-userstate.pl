...
$db->do("DROP TABLE IF EXISTS UserState");
$db->do("DROP TABLE IF EXISTS CloneTwilio ");
$db->do("DROP TABLE IF EXISTS Scene");

$db->do("CREATE TABLE Clone (CloneID INTEGER PRIMARY KEY, CloneName TEXT, UserName TEXT, Password TEXT)");
...
#$db->do("alter table Groups add column SceneID INTEGER default 1");
#$db->do("alter table Number add column CloneID INTEGER default 1");
#$db->do("alter table TwilioNumber add column CloneID INTEGER default 1");
